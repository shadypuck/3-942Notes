...
\section{Conformation: Real Chains}
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item We'll derive the "easy force response of a freely jointed chain."
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{itemize}
        \end{itemize}
        \item Now that we know the number of ways we can move a distance $x$, we can calculate the probability $P(x)$ of moving a distance $x$ by dividing by the total number of possible paths (which is just $\Omega_\text{tot}=2^n$).
        \begin{equation*}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item $U$ is internal potential energy;
        ...
        \end{equation*}
        \begin{itemize}
            \item We'll denote the denominator by $\Omega_N$ going forward, representing the fact that it is a function of $N$ and not $\mathbf{R}$.
        \end{itemize}
        \item Then substituting and bashing out the math, we get
        \begin{align*}
            S(\mathbf{R},N) &= \kB\ln[P(\mathbf{R},N)]+\kB\ln(\Omega_N)\\
            &= -\frac{3\kB R^2}{2Nb^2}+\kB\ln(\frac{3}{2\pi Nb^2})^{3/2}+\kB\ln(\Omega_N)\\
            &= -\frac{3\kB R^2}{2Nb^2}+S_N
        \end{align*}
        \begin{itemize}
            \item We will call the second two terms $S_N$ because they're less interesting, and they only depend on $N$ (not $\mathbf{R}$).
        \end{itemize}
        \item By substituting the above back into $F=U-TS$, it follows that our entropic free energy for the FJC is
        \begin{equation*}
    ...
    \begin{itemize}
        ...
        \item Non-local elements are related by excluded volume. Essentially, monomers may have some interaction (favorable or unfavorable) when they float near each other in solution.
    \end{itemize}
    \item In ideal chains\dots
    \begin{itemize}
        \item There are only local effects;
        \item Ideal is synonymous with either theta conditions or melts.
    \end{itemize}
    ...
    \end{figure}
    \begin{itemize}
        \item For nonlocal interactions, we will consider "hard sphere" monomers.
        \begin{itemize}
            \item In particular, for two hard-sphere monomers with center-to-center distance $\delta$, Figure \ref{fig:monomerGasb} tells us that excluded volume $V_\text{ex}=v$ is
        \begin{equation*}
            V_\text{ex} = \frac{4}{3}\pi\delta^3
        \end{equation*}
        \end{itemize}
        \item Let's generalize: The Boltzmann weighting for the probability of finding two monomers at a distance $r$ from each other is
        \begin{equation*}
            f(r) = \exp(-\frac{U(r)}{\kB T})-1
        \end{equation*}
        \begin{itemize}
            \item $f(r)$ is called the \textbf{Mayer-$\bm{f}$ function}.
            \item It is equal to zero when there is no interaction (i.e., $U=0$) because when $U=0$, all $r$ are equally probable.
        \end{itemize}
        \item It follows through some math --- see \textcite{bib:RubinsteinColby} --- that the excluded volume is equal to
        \begin{equation*}
            V_\text{ex} = -4\pi\int_0^\infty r^2f(r)\dd{r}
        \end{equation*}
        \begin{itemize}
            \item Notice that we're in polar coordinates.
            \item What is going on here?? Why is excluded volume related to the probability of finding monomers near each other?
        \end{itemize}
        \item Note that $U(r)$ (which is the monomer interaction) also incorporates the solvent since the energy is not that of two bare monomers, but those monomers with a medium between them (e.g., two styrene monomers in a sea of toluene).
        \item The excluded volume is a way of quantifying interactions.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item In a good solvent, the coils unwind and the volume expands. Repulsion dominates (Figure \ref{fig:MayerFa}).
        ...
        \end{itemize}
        \item We can also plot out precursors to and variants of the Mayer-$f$ function for specific potentials of interest.
        \begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.25\linewidth]{compressPenalty.JPG}
    ...
    \begin{itemize}
        ...
        \item We say concentration is approximately equal to the number of monomers divided by $R^3$ (throwing out prefactors like $4/3$ and $\pi$).
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    \item The full Flory free energy has many terms that all balance; you play with the ingredients to see what fits.
    \begin{equation*}
