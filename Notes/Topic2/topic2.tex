...
\section{Flory-Huggins Theory}\label{sse:5}
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{equation*}
            \item This equation tells us that when $N$ is big, entropy doesn't drive mixing much (because $N_1,N_2$ are in the denominator). Thus, enthalpy matters more for the mixing of polymers.
        \end{itemize}
    \end{itemize}
...
\section{The Theta State}\label{sse:6}
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Recall from math class that for small, positive $x$, we have the following Taylor series expansion.
        \begin{equation*}
            \ln(1-x) = -x-\frac{x^2}{2}-\cdots-\frac{x^n}{n}-\cdots
        \end{equation*}
        \begin{itemize}
            \item Additionally, we can approximate $\ln(1-x)$ for sufficiently small $x$ by choosing only the first couple of terms in the expansion.
        \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{equation*}
        \end{itemize}
        \item When contact interactions and excluded volume balance, this is the $\theta$ condition. It follows that the $\theta$ condition occurs when the excess chemical potential equals zero. But mathematically, this happens when $\chi=1/2$!
        \begin{itemize}
    ...
    \end{itemize}
    \item Let's now discuss how we can estimate $\chi$ experimentally using \textbf{solubility parameters}.
    \begin{itemize}
        \item How do you determine $\chi$ for a mixture? Hildebrand's interesting idea was to use the molar enthalpy of vaporization $\Delta\hat{H}_{v,i}$ of each component $i$ in the mixture.
        \item Disclaimer: This derivation works better for nonpolar substances than polar substances; you will see why shortly.
    \end{itemize}
    \item Aside (review): Some thermodynamics where the difference between enthalpy $H$ and energy $U$ actually does matter.
    \begin{itemize}
        \item So far in this class, we have largely ignored the difference between enthalpy and energy, using the two terms interchangeably.
        \item However, in Hildebrand's setup, we will experimentally measure the molar enthalpy of vaporization $\Delta\hat{H}_{v,i}$ while theoretically calculating the molar energy of vaporization $\Delta\hat{U}_{v,i}$. Thus, we need to relate these two quantites.
        \item Recall from thermodynamics that enthalpy $H=U+PV$ is defined as the state function describing heat put into a system at constant pressure. It follows (since pressure is constant) that
        \begin{equation*}
            \Delta H = \Delta(U+PV) = \Delta U+\Delta(PV) = \Delta U+P\Delta V
        \end{equation*}
        \item In the specific case of vaporization,
        \begin{equation*}
            \Delta H_v = \Delta U_v+[(PV)_{(g)}-(PV)_{(l)}]
        \end{equation*}
        \item From the ideal gas law, $PV=nRT$. Moreover, since gases occupy far more volume than liquids, we may neglect $(PV)_{(l)}$ and assume that $P\Delta V\approx(PV)_{(g)}$. Thus,
        \begin{equation*}
            \Delta H_v = \Delta U_v+nRT
        \end{equation*}
        \item Finally, in the specific case where we are considering only 1 mole of a substance,
        \begin{align*}
            \Delta\hat{H}_v &= \Delta\hat{U}_v+(1)RT\\
            \Delta\hat{H}_v &= \Delta\hat{U}_v+RT
        \end{align*}
    \end{itemize}
    \item Hildebrand's experimental setup and derivation.
    \begin{figure}[h!]
        \centering
    ...
    \end{figure}
    \begin{itemize}
        \item Take a pure liquid, heat it up, and measure how much heat goes into the system before it turns into a gas.
        \item Consider this experiment from the perspective of lattice theory.
        \begin{itemize}
            \item In lattice theory, every particle in a liquid corresponds to a point in the lattice. These lattice points interact with each other via enthalpic nearest neighbor interactions, as discussed above extensively (see, for example, Figures \ref{fig:nearNeighbor}-\ref{fig:Uwell}).
            \item But when you transition to the gas phase, every particle is now neighborless. Thus, in terms of lattice theory, nearest neighbor interactions are gone and no particle has any potential energy.
            \item Essentially, in terms of lattice theory, the process of \emph{vaporization} is equivalent to overcoming each nearest neighbor interaction!
        \end{itemize}
        \item Let's now investigate these nearest neighbor interactions a bit more.
        \begin{itemize}
            \item Last lecture, we counted each nearest neighbor interaction in a pure substance, and denoted the total by
            \begin{equation*}
                \xi_{ii} = \frac{X_iz\phi_i}{2}
            \end{equation*}
            \item In this scenario (i.e., a pure component), the volume fraction $\phi_i=1$.
            \item Additionally, since it will be easiest to measure the vaporization of a \emph{molar} quantity, we will be interested in finding the number of nearest neighbor interactions in \emph{one mole} of a pure substance. Thus, we set the number of particles $X_i$ equal to the number of particles in a mole, which is just Avogadro's number $N_\text{A}$.
            \item Therefore, the total number of nearest neighbor interactions broken during the vaporization of one mole of a pure substance is
            \begin{equation*}
                \frac{N_\text{A}z}{2}
            \end{equation*}
        \end{itemize}
        \item Continuing to let each nearest neighbor energetic interaction have magnitude $\varepsilon_{ii}$, we learn that the molar energy of vaporization $\Delta\hat{U}_{v,i}$ of component $i$ is equal to the number of interactions broken times the magnitude of each interaction, or
        \begin{equation*}
            \Delta\hat{U}_{v,i} = \hat{U}_{(g),i}-\hat{U}_{(l),i}
            = 0-\frac{N_\text{A}z\varepsilon_{ii}}{2}
            = -\frac{N_\text{A}z\varepsilon_{ii}}{2}
        \end{equation*}
        \begin{itemize}
            \item Note that since $\varepsilon_{ii}<0$, we have that $\Delta\hat{U}_{v,i}>0$.
        \end{itemize}
        \item From this equation, we may define the \textbf{cohesive energy density} and \textbf{solubility parameter}.
    \end{itemize}
    \item \textbf{Cohesive energy density}: The energy needed to make a pure substance $i$ cohere, or stick together, per unit volume. \emph{Denoted by} $\textbf{CED}$. \emph{Given by}
    \begin{equation*}
        \text{CED} := \frac{\Delta\hat{U}_{v,i}}{\hat{V}_i}
        = \frac{\Delta\hat{H}_{v,i}-RT}{\hat{V}_i}
    \end{equation*}
    \begin{itemize}
        \item $\hat{V}_i$ denotes the molar volume of substance $i$ (i.e., the volume occupied by one mole of substance $i$).
        \item Note that in this case, the slides use $E$ instead of $U$ to denote energy.
    \end{itemize}
    \item \textbf{Solubility parameter}: A numerical estimate of the degree of interaction between materials. \emph{Units} \textbf{\si[text-series-to-math]{\raiseto{1/2}\mega\pascal}}. \emph{Denoted by} $\bm{\delta}$. \emph{Given by}
    \begin{equation*}
        \delta := \sqrt{\text{CED}}
    \end{equation*}
    \begin{itemize}
        \item Materials with similar solubility parameters are likely to be miscible.
    \end{itemize}
    \item Let's now continue the derivation of the \textbf{Hildebrand equation}.
    \begin{itemize}
        \item We'll focus on a binary mixture of two components, named 1 and 2.
        \item Let $v$ denote the volume of a single lattice site. Then from the above definitions, the solubility parameter $\delta_i$ of each component in a binary mixture is
        \begin{equation*}
            \delta_i = \sqrt{-\frac{N_\text{A}z\varepsilon_{ii}}{2\hat{V}_i}}
            = \sqrt{-\frac{N_\text{A}z\varepsilon_{ii}}{2(N_\text{A}v)}}
            = \sqrt{-\frac{z\varepsilon_{ii}}{2v}}
        \end{equation*}
        \item Since 1 and 2 are assumed to be nonpolar, they attract (to themselves and to each other) only through London dispersion forces. In this case, the relevant attraction equation is
        \begin{equation*}
            \varepsilon_{ij} = -\frac{\alpha_1\alpha_2}{r^6}
        \end{equation*}
        \begin{itemize}
            \item We assume that the elements of component $i$ and component $j$ are separated by a distance $r$, as in Figure \ref{fig:monomerGasc}. Specifically, $r$ will be equal to the distance between two cells in the lattice.
            \item Notice that the attraction energy scales as $1/r^6$, and is also dependent on the \textbf{polarizability} / \textbf{polarizability volumes}.
            \item Origin of the above relation: Related to dipole-induced dipole attractions; when you average over all possible combinations, this relation falls out. And that's what Lennard and Jones based their use of $1/r^6$ as the attractive term on!
            \item In PSet 2, we will prove that this attraction rule is "like likes like."
        \end{itemize}
        \item It follows from the above that
        \begin{equation*}
            \varepsilon_{12} = -\sqrt{\varepsilon_{12}^2}
            = -\sqrt{\frac{\alpha_1^2\alpha_2^2}{r^{12}}}
            = -\sqrt{\left( -\frac{\alpha_1\alpha_1}{r^6} \right)\left( -\frac{\alpha_2\alpha_2}{r^6} \right)}
            = -\sqrt{\varepsilon_{11}\varepsilon_{22}}
        \end{equation*}
        \begin{itemize}
            \item The above equation is known as \textbf{Berthelot's mixing rule}.
            \item It expresses $\varepsilon_{12}$ as the \textbf{geometric mean} of $\varepsilon_{11},\varepsilon_{22}$.
        \end{itemize}
        \item In light of the above definitions and equations, we may write
        \begin{align*}
            (\delta_1-\delta_2)^2 &= \delta_1^2-2\delta_1\delta_2+\delta_2^2\\
            &= -\frac{z\varepsilon_{11}}{2v}-2\sqrt{\frac{z\varepsilon_{11}}{2v}\cdot\frac{z\varepsilon_{22}}{2v}}-\frac{z\varepsilon_{22}}{2v}\\
            &= \frac{z}{v}\left[ -\sqrt{\varepsilon_{11}\varepsilon_{22}}-\frac{1}{2}(\varepsilon_{11}+\varepsilon_{22}) \right]\\
            &= \frac{z}{v}\left[ \varepsilon_{12}-\frac{1}{2}(\varepsilon_{11}+\varepsilon_{22}) \right]
        \end{align*}
        \item Now just multiply by $v/\kB T$ to get the $\chi$ parameter:
        \begin{equation*}
            \frac{v}{\kB T}(\delta_1-\delta_2)^2 = \frac{v}{\kB T}\cdot\frac{z}{v}\left[ \varepsilon_{12}-\frac{1}{2}(\varepsilon_{11}+\varepsilon_{22}) \right]
            = \underbrace{\frac{z}{\kB T}\left[ \varepsilon_{12}-\frac{1}{2}(\varepsilon_{11}+\varepsilon_{22}) \right]}_\chi
        \end{equation*}
        \begin{itemize}
            \item Since $\chi$ is equal to the product of positive quantities (including a perfect square), this equation shows that $\chi\geq 0$ for a mixture of nonpolar substances!
        \end{itemize}
        \item From here, we may finally get to the \textbf{Hildebrand equation}, which is the last line below.
        \begin{align*}
            \frac{\Delta H_M}{X_0\kB T} &= \chi\phi_1\phi_2\\
            \frac{\Delta H_M}{X_0\kB T} &= \phi_1\phi_2\cdot\frac{v}{\kB T}(\delta_1-\delta_2)^2\\
            \Delta\hat{H}_M &= \hat{V}\phi_1\phi_2(\delta_1-\delta_2)^2 \geq 0\tag{Hildebrand equation}
        \end{align*}
        \begin{itemize}
            \item $\Delta\hat{H}_M$ is the molar enthalpy of mixing. We get from the 2nd to the third line by multiplying through by $N_\text{A}\kB T$.
            \item $\hat{V}$ is the average molar volume of solvent / monomers.
            \begin{itemize}
                \item Note that one of the assumptions of lattice theory is that $\hat{V}=\hat{V}_1=\hat{V}_2$.
            \end{itemize}
            \item This equation justifies why substances with similar solubility parameters are more likely to mix well, as stated in the definition of the solubility parameter.
            \begin{itemize}
                \item Since $\chi\geq 0$, the enthalpy of mixing (for two nonpolar substances) will always be nonnegative. Thus, enthalpy can only cause substances to demix, not to mix better.
                \item However, by the form of the Hildebrand equation, we see that as $\delta_1\to\delta_2$ (i.e., as $|\delta_1-\delta_2|\to 0$), the enthalpy of mixing will shrink. This makes entropy-driven mixing much more likely.
                \item Therefore, similar solubility parameters decrease the enthalpic barrier to mixing!
            \end{itemize}
        \end{itemize}
...
\section{Phase Behavior and Microphase Separation}\label{sse:7}
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Principle in biology: It's more useful to do multiple tasks poorly than one task really well.
        \begin{itemize}
        ...
        \end{itemize}
        \item Consider a (biochemical) network, with control over each local internode transition (Figure \ref{fig:bioNetwork}).
        \begin{itemize}
        ...
        \end{itemize}
        \item Takeaway: Randomness affords capabilities beyond what's possible with perfectly folded proteins.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{Intra interactions} $\chi_{AB}$, $\chi_{AC}$, and $\chi_{BC}$ between each component of the chain.
            \item Chain length: Let the chain have total length $N$, and let each of the three segments have length $N_i$. Then we have $N_A$, $N_B$, and $N_C=N-N_A-N_B$.
        \end{itemize}
    ...
    \end{itemize}
    \item If we can do everything in Figure \ref{fig:selfAssembly} with 3 things, imagine what's possible with 20 amino acids!
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Aside: In rogue prions, exposed $\beta$-pleated sheets stack vertically with the $\beta$-pleated sheets of other prions. This stacking is what causes brain shut down in Mad Cow Disease.
    \end{itemize}
    \item Goals for self-assembly.
    \begin{itemize}
        \item Understand the key concepts behind self-assembly, in particular for the case of block copolymers.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item This interface is technically termed the \textbf{inter-material dividing surface} (or IMDS).
        \end{itemize}
        ...
        \begin{itemize}
            \item $\chi N$ controls whether or not we develop microdomains (more on this below, and in PSet 2).
        \end{itemize}
    \end{itemize}
    ...
    \end{figure}
    \begin{itemize}
        \item $T_\text{ODT}$ denotes the order-disorder transition temperature.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            \footnotesize
            \begin{scope}
                \draw [dashed,semithick] (0,-0.5) -- (0,2.4) node[above]{IMDS};

                \draw [blx,thick] (0,0)
                    -- ++(150:0.4)
                    -- ++(190:0.4) coordinate (amid)
                    -- ++(165:0.4) coordinate (a1)
                    -- ++(240:0.4) coordinate (a2)
                ;
                \draw [rex,thick] (0,0)
                    -- ++(-9:0.4)
                    -- ++(8:0.4) coordinate (bmid)
                    -- ++(-39:0.4)
                    -- ++(43:0.4)
                ;
                \draw [blx,thick] (0,1)
            ...
            \draw [|-|] (a1) ++(150:0.2) -- node[left=-1pt,yshift=2pt]{$a$} ++(240:0.4);
            \draw [|-|] (-1.55,-1) -- node[below]{$\lambda/2$} ++(3.1,0);
            \draw [|-|] (-3.1,-1.5) -- node[below]{$\lambda$} ++(6.2,0);
            \fill [grx,opacity=0.5] (0,0) ellipse (3pt and 5mm) node[right=1pt,yshift=3mm,black,opacity=1]{$\text{area}=\Sigma$};
            \node [below=2mm] at (amid) {$N_\text{A}$};
            \node [below=1mm] at (bmid) {$N_\text{B}$};
        \end{tikzpicture}
        \caption{Variables describing microdomain spacing.}
        \label{fig:microdomainVariables}
    \end{figure}
    \begin{itemize}
        \item $G$ is the free energy \emph{per chain}.
        \item $N=N_A+N_B$ is the number of segments per chain.
        \begin{itemize}
            \item In Figure \ref{fig:microdomainVariables} as drawn, $N_\text{A}=N_\text{B}=4$.
        \end{itemize}
        \item $a$ is the step size.
        \begin{itemize}
            \item This can mean the Kuhn length or bond length, varying depending on the context.
            \item For the purposes of this derivation, we assume that the step size for block A is equal to the step size for block B.
        \end{itemize}
        \item $\lambda$ is the domain periodicity.
        \begin{itemize}
            \item This is the length of one of the repeating periods in a lamellar structure.
            \item In the context of Figure \ref{fig:microphaseMorph}'s green-yellow-green-yellow lamellae, $\lambda$ is the width of one pair of green-yellow lamellae back to back. You can think of this as a kind of "repeat unit," the smallest structure length necessary such that upon repetition of that structure, the full lamellar mesophase is formed.
            \item This also means that the left sketch in Figure \ref{fig:diblockStretch} is mislabeled: It should be $\lambda/2$, which is also the expected length of a single chain in the microphase separated state.
        \end{itemize}
        \item $\Sigma$ is the interfacial area where a single chain's A block interacts with its B block.
        \item $\gamma_\text{AB}$ is the interfacial energy on the IMDS per unit area.
        \begin{itemize}
            \item We compute this using \textbf{Helfand's equation}.
        \end{itemize}
        \item $\chi_\text{AB}$ is the same Flory-Huggins interaction parameter we've been looking at in previous lectures, i.e., between homopolymers of A and B.
    \end{itemize}
    \item \textbf{Helfand's equation}: The following expression of the interfacial energy in terms of other variables. \emph{Given by}
    \begin{equation*}
        \gamma_\text{AB} = \frac{\kB T}{a^2}\sqrt{\frac{\chi_\text{AB}}{6}}
    \end{equation*}
    \item To get a little more comfortable working with these variables, let's discuss the following useful mathematical relation between them.
    \begin{equation*}
        Na^3 = \Sigma\cdot\frac{\lambda}{2}
    \end{equation*}
    \begin{itemize}
        \item This essentially equates (1) the total volume occupied by $N$ monomers, each of volume $a^3$ and (2) the volume of the cylinder bounding said monomers, a cylinder having height $\lambda/2$ and base area $\Sigma$.
    \end{itemize}
    \item We now calculate the change in free energy of a diblock copolymer chain as it phase separates from a melt state and into microdomains.
    \begin{figure}[h!]
        \centering
        \begin{subfigure}[b]{0.2\linewidth}
            \centering
            \includegraphics[width=0.6\linewidth]{diblockStatea.png}
            \caption{Melt state.}
            \label{fig:diblockStatea}
        \end{subfigure}
        \begin{subfigure}[b]{0.2\linewidth}
            \centering
            \includegraphics[width=0.9\linewidth]{diblockStateb.png}
            \caption{Microdomains.}
            \label{fig:diblockStateb}
        \end{subfigure}
        \caption{Diblock copolymer states.}
        \label{fig:diblockState}
    \end{figure}
    \begin{itemize}
        \item Let State 0 denote pure component A or B, State 1 denote a homogeneous melt of AB diblock copolymers (Figure \ref{fig:diblockStatea}), and State 2 denote a phase separation of AB diblock copolymers into microdomains (Figures \ref{fig:diblockStateb} and \ref{fig:microdomainVariables}).
        \begin{itemize}
            \item In a homogeneous melt, excluded volume effects are not present. Thus, in State 1, a single polymer chain's end-to-end distance $R=N^{1/2}a$.
            \item By the definition of $\lambda$, a single polymer in State 2 has on average an end-to-end distance of $R=\lambda/2$.
        \end{itemize}
        \item We want to calculate the change $\Delta G$ in Gibbs free energy \emph{per polymer chain} upon transitioning from State 1 to State 2. In terms of enthalpy and entropy, $\Delta G$ is thus given by
        \begin{equation*}
            \Delta G = (\underbrace{H_2-H_1}_{\Delta H})-T(\underbrace{S_2-S_1}_{\Delta S})
        \end{equation*}
        \item We have already developed extensive theory for polymers in the melt state. Let's use some of it to calculate each $H_i$ or $S_i$ in the above equation.
        \begin{itemize}
            \item Recall from Lecture \ref{sse:4} that the entropy $S$ of a polymer chain without excluded volume effects (as in the melt state) is given by
            \begin{equation*}
                S = -\frac{3\kB R^2}{2Na^2}
            \end{equation*}
            \item Recall from Lecture \ref{sse:5} that the mixing energy per site $\Delta H_M/X_0$ of components A and B may be related to other parameters by
            \begin{equation*}
                \frac{\Delta H_M}{X_0\kB T} = \chi_\text{AB}\phi_\text{A}\phi_\text{B}
            \end{equation*}
            \begin{itemize}
                \item Since there are $N$ sites in a polymer chain, the mixing energy \emph{per chain} of components A and B is
                \begin{equation*}
                    \Delta H_M = N\kB T\chi_\text{AB}\phi_\text{A}\phi_\text{B}
                \end{equation*}
                \item Lastly, by our definitions of States 0-2, the mixing energy is equal to the difference in energy between a chain in State 1 and a chain in State 0. Note that this assumes that State 1 is basically the same as a melt of homopolymers of component A and component B. In this case, we have
                \begin{equation*}
                    H_1-H_0 = N\kB T\chi_\text{AB}\phi_\text{A}\phi_\text{B}
                \end{equation*}
            \end{itemize}
            \item Lastly, let's use some of our new variables.
            \begin{itemize}
                \item In both State 0 and State 2, like chains will interact with each other enthalpically via at least London dispersion forces. Recall that this is how we got the equations $H_i=zX_i\varepsilon_{ii}/2$ in Lecture \ref{sse:5}.
                \item However, in State 2, there is an additional enthalpic interaction: The interfacial energy on the IMDS. Since we have this energy per unit area on the IMDS ($\gamma_\text{AB}$) and the interfacial area of a single chain ($\Sigma)$, we can obtain the interfacial energy on the IMDS \emph{per chain} by taking the product $\gamma_\text{AB}\Sigma$.
                \item Thus, the difference in energies between states 0 and 2 is purely equal to this additional enthalpic interaction. Symbolically,
                \begin{equation*}
                    H_2-H_0 = \gamma_\text{AB}\Sigma
                \end{equation*}
            \end{itemize}
        \end{itemize}
        \item Thus, putting everything together, we obtain
        \begin{align*}
            \Delta G &= [(H_2-H_0)-(H_1-H_0)]-T\left[ S_2-S_1 \right]\\
            &= [\gamma_\text{AB}\Sigma-N\kB T\chi_\text{AB}\phi_\text{A}\phi_\text{B}]-T\left[ -\frac{3\kB(\lambda/2)^2}{2Na^2}+\frac{3\kB(N^{1/2}a)^2}{2Na^2} \right]\\
            &= \underbrace{\vphantom{\frac{3}{2}\kB T\left[ \frac{(\lambda/2)^2}{Na^2}-1 \right]}\gamma_\text{AB}\Sigma-N\chi_\text{AB}\phi_\text{A}\phi_\text{B}\kB T}_\text{Enthalpic Term}+\underbrace{\frac{3}{2}\kB T\left[ \frac{(\lambda/2)^2}{Na^2}-1 \right]}_\text{Entropic Term}
        \end{align*}
        \begin{itemize}
            \item The entropic term quantifies the springiness of the polymer. It is an important observation that $\Delta S\propto\lambda^2/Na^2$.
        \end{itemize}
    \end{itemize}
    \item Now that we have an equation for the change in energy upon microdomain formation, what can we do with it? One useful thing would be to calculate the optimal domain periodicity $\lambda_\text{opt}$.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.35\linewidth]{diblockStretch.JPG}
        \caption{Structural changes as diblock copolymers are stretched.}
        \label{fig:diblockStretch}
    \end{figure}
    \begin{itemize}
        \item The idea that such a $\lambda_\text{opt}$ exists is based in an important assumption: that polymer chains will want to stretch away from the IMDS.
        \item Enthalpy goes way down when you have linear strands (as on the right side of Figure \ref{fig:diblockStretch}), since this puts most monomers closer to each other on average than they are to monomers of the opposite component.
        \begin{itemize}
            \item Similarly, enthalpy goes way up when everything squishes near the IMDS (as on the left side of Figure \ref{fig:diblockStretch}). This is because in this case, the interfacial area per chain $\Sigma$ gets way bigger.
        \end{itemize}
        \item In contrast, entropy goes way up when you have linear strands because we're stretching.
        \item Thus, the system will try to find the optimal balance between the enthalpic and entropic forces, by which we mean that we want to optimize the length $\lambda$.
    \end{itemize}
    \item Let's now begin calculating $\lambda_\text{opt}$.
    \begin{itemize}
        \item To find the optimal $\lambda$, we'll want to find the minimum change in Gibbs free energy upon microphase separation as a function of $\lambda$. Symbolically, we want to minimize the following equation.
        \begin{equation*}
        ...
        \end{equation*}
        \item Compressing every non-$\lambda$ variable in the above expression into constants (termed $\alpha$, $\beta$, $\text{const}_1$, or $\text{const}_2$) reveals that the above equation is of the following general form.
        \begin{equation*}
            \Delta G(\lambda) = \frac{\alpha}{\lambda}-\text{const}_1+\beta\lambda^2-\text{const}_2
        \end{equation*}
        \item Thus, the optimal period of the lamellae repeat unit is
        \begin{align*}
            0 &= \eval{\pdv{\Delta G}{\lambda}}_{\lambda_\text{opt}}\\
            &= -\frac{\alpha}{\lambda_\text{opt}^2}+2\beta\lambda_\text{opt}\\
            \lambda_\text{opt} &= \sqrt[3]{\frac{\alpha}{2\beta}}\\
            &= \Bigg[ \frac{1}{2}\bigg( \underbrace{\frac{2\kB T\sqrt{\chi_\text{AB}}Na}{\sqrt{6}}}_\alpha \bigg)\bigg( \underbrace{\vphantom{\frac{2\kB T\sqrt{\chi_\text{AB}}Na}{\sqrt{6}}}\frac{3\kB T}{8Na^2}}_\beta \bigg)^{-1} \Bigg]^{1/3}\\
            &= \left[ \left( \frac{32}{27} \right)^{1/2}a^3N^2\chi_\text{AB}^{1/2} \right]^{1/3}\\
            % &\approx 1.03aN^{2/3}\chi_\text{AB}^{1/6}\\
            \lambda_\text{opt} &\approx aN^{2/3}\chi_\text{AB}^{1/6}
        \end{align*}
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{align*}
        \begin{itemize}
            \item Since the first two terms in line 3 are both much greater than the third term, we neglect it.
            \item Thus, the sign of $\Delta G$ will depend on which of the two terms remaining in line 4 is bigger.
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \end{equation*}
            \item It follows that the critical $N\chi$ value $(N\chi)_c$ --- at which $\Delta G(\lambda_\text{opt})=0$ --- is
            \begin{align*}
    ...
    \end{figure}
    \begin{itemize}
        \item As expected, $(\chi N)_c\approx 10.5$.
        \item Moreover, as we go in from the outside (homogeneous, disordered mixed melt), we progressively go through spheres (S), cylinders (C), gyroids (G), and lamellae (L).
    \end{itemize}
    \item Block copolymer microphases locally look like a series of homopolymers bound to the planar IMDS. This construction is actually very much related to another scenario known as a \textbf{polymer brush}.
    \item \textbf{Polymer brush}: A series of homopolymers which have been attached by one end to a hard, planar surface.
    \begin{itemize}
        \item See Figure \ref{fig:PSet2Q5setup} for a visualization.
    \end{itemize}
    \item Variables relevant to polymer brushes.
    \begin{itemize}
        \item $D$ is the average distance between each grafting site.
        \item $H$ is the average height away from the surface to which the polymers stretch.
        \begin{itemize}
            \item It follows from the definitions of $D$ and $H$ that the volume occupied by a single polymer chain is $HD^2$.
        \end{itemize}
        \item As before, $N$ and $a$ are Kuhn parameters.
        \begin{itemize}
            \item Thus, analogously to before, $HD^2=Na^3$.
        \end{itemize}
        \item $c=N/HD^2$ is the (assumed uniform) number density of monomers throughout the brush's volume.
        \item $n$ is the total number of chains on the surface.
        \item $A$ is the total area of the surface.
        \item $\sigma=\rho=n/A=1/D^2=H/Na^3$ is the grafting density.
    \end{itemize}
    \item The polymer strands stretch out more ($H$ increases) in two main cases.
    \begin{itemize}
        \item When they don't want to interact with the 2D interface.
        \item When they are so close together that they are crowded away from the surface.
    \end{itemize}
    \item Now that we have a language through which to discuss brushes, let's analyze the system as before.
    \begin{itemize}
        \item What is the energy or enthalpy? If we need to take excluded volume into account, then from Lecture \ref{sse:4}, 
        \begin{equation*}
            \Delta H = \frac{\kB T}{2}vc^2V_p \propto vc^2\cdot HD^2 = \frac{vN^2}{HD^2}
        \end{equation*}
        \begin{itemize}
            \item $V_p=HD^2=Na^3$ is the volume of a single polymer chain.
            \item $v$ is the excluded volume.
            \item $c^2$ describes how dense the system is.
            \begin{itemize}
                \item Explicitly, it is the probabilty of two segments coming into contact.
            \end{itemize}
            \item Together, $vc^2$ describes how many two body interactions there are (counted by mole).
        \end{itemize}
        \item What is the entropy?
        \begin{equation*}
            \Delta S \propto \frac{H^2}{Na^2}
        \end{equation*}
        \begin{itemize}
            \item This is analogous to the $\lambda^2/Na^2$ proportionality from the diblock copolymer case, except with variables changed.
            \item For the full expression for $\Delta S$, the entropy derivation from above is exactly analogous, affording
            \begin{equation*}
                \Delta S = -\frac{3}{2}\kB\left( \frac{H^2}{Na^2}-1 \right)
            \end{equation*}
        \end{itemize}
        \item Now we may consider how to find the optimal stretching distance $H$.
        \begin{itemize}
            \item The enthalpic term goes as $1/H$ and the entropic term goes as $H^2$. Thus, the enthalpic term increases at small $H$ and the entropic term increases at large $H$.
            \item Therefore, the Gibbs free energy goes as $H^2+1/H$ and has a proper energy valley. It is the minimum point on this valley that we seek.
            \item We will explore this problem more in PSet 2, Q5.
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \marginnote{9/30:}PSet 2, Q5 (\emph{warning: much in here is not entirely accurate}).
    \begin{figure}[h!]
        \centering
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            \Delta G = \frac{H^2}{Na^2}+V_\text{ex}\rho^2\cdot HD^2-\varepsilon
        \end{equation*}
        \begin{itemize}
            \item $HD^2$ is a volume.
            \item $\rho=N/HD^2$ is the number density of monomers.
            \item So then we need the optimal $H$, which we will term $H^*$. $H^*$ minimizes $\Delta G(H^*)$, i.e., $\ev{\pdv*{\Delta G}{H}}_{H=H^*}=0$.
        ...
        \begin{equation*}
            \Delta G_2-\Delta G_1 = \frac{(H^*)^2}{Na^2}+\frac{vN^2}{H^*D^2}-\varepsilon-\mu_\text{bulk}
        \end{equation*}
        \begin{itemize}
        ...
        \begin{itemize}
            \item $vN^2/HD^2$.
            \item Stretching term is $H^2/Na^2$. Excluded volume is $v\rho^2\cdot HD^2$.
            \item Balance the two terms $H^2/Na^2$ and $vN^2/HD^2$ to find $H^*$.
...
\begin{itemize}
    ...
    \end{itemize}
    \item What are some good resources to read about phase behavior in block copolymers, attachment to surfaces, interfacial energy, and the other topics from Lecture \ref{sse:7} that are not covered in \textcite{bib:HiemenzLodge}?
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    \item \textbf{Molar} (quantity $Y$ of substance $i$): The amount of $Y$ contributed by each mole of a substance $i$ when pure. \emph{Denoted by} $\bm{\hat{Y}_i}$. \emph{Units} $\textbf{mol}^{\bm{-1}}$.
    ...
    \end{itemize}
    \item "As a disperse polymer solution is cooled below the theta point, the higher molecular weight chains will tend to phase separate first, a feature which can be used to advantage in fractionation" \parencite[299]{bib:HiemenzLodge}.
    ...
    \end{equation*}
    \item \marginnote{10/10:}\textcite[303]{bib:HiemenzLodge} covers several possible definitions of a reference volume for one monomer.
    \begin{itemize}
        \item One possibility is a geometric mean of the molar volumes of the repeat units. Why the geometric mean??
        \item Another is to take \SI{100}{\cubic\angstrom} as a generally good number.
    \end{itemize}
    \item \textcite[306]{bib:HiemenzLodge} has a table of solubility parameters, but in non-SI units.
    \item \marginnote{10/12:}For the specific case of polymer-solvent solutions, solubility parameter-based formulations of $\chi$ differ significantly from experimental results.
    \begin{itemize}
        \item A related model that is empirically more accurate is
        \begin{equation*}
            \chi \approx 0.34+\frac{v}{\kB T}(\delta_1-\delta_2)^2
        \end{equation*}
        \item In particular, our theory often \emph{underestimates} $\chi$ for polymer-solvent solutions, so we need to add a temperature-independent correction.
        \begin{itemize}
            \item This means that in reality, each component likes itself more (and the other component less) than regular solution theory would predict.
        \end{itemize}
        \item This correction reflects the finding that the entropy of mixing in polymer-solvent solutions is smaller than regular solution theory would suggest, since it implies that
        \begin{align*}
            \frac{\Delta G_M}{X_0} &= \kB T(\chi_\text{corrected}\phi_1\phi_2+\phi_1\ln\phi_1+\phi_2\ln\phi_2)\\
            &= \kB T(\underbrace{\chi_\text{RST}\phi_1\phi_2}_\text{enthalpic}+\underbrace{0.34\phi_1\phi_2+\phi_1\ln\phi_1+\phi_2\ln\phi_2}_\text{entropic})
        \end{align*}
        \begin{itemize}
            \item $\chi_\text{corrected}$ is the new empirical formula for $\chi$, and $\chi_\text{RST}$ is the value expected from Hildebrand's derivation using only \underline{r}egular \underline{s}olution \underline{t}heory.
            \item As we can see, the new (strictly positive) entropic term makes the overall entropy of mixing less negative than when only the former (strictly negative) terms were present.
        \end{itemize}
    \end{itemize}
    \item Additional empirical issues with the regular solution theory approach to polymer-solvent solutions.
    \begin{itemize}
        \item Some behavior is wildly different than just having a single LCST and/or UCST.
        \item Some mixtures contain polar species, which do not satisfy the purely combinatorial assumptions of regular solution theory.
    \end{itemize}
    \item One commonly used theoretical solution to the polymer-solvent solution conundrum: The \textbf{effective interation parameter}.
    \item \textbf{Effective interaction parameter}: An interaction parameter that quantifies a solution's deviation from ideality. \emph{Denoted by} $\bm{\chi_\textbf{eff}}$. \emph{Given by}
    \begin{equation*}
        \chi_\text{eff} := \frac{\alpha}{T}+\beta = \chi_h+\chi_s
    \end{equation*}
    \begin{itemize}
        \item $\chi_h=\alpha/T$ is is the enthalpic contribution to $\chi_\text{eff}$.
        \item $\chi_s=\beta$ is is the entropic contribution to $\chi_\text{eff}$.
        \item $\alpha,\beta\in\R$.
        \begin{itemize}
            \item $\alpha<0$ implies an attractive interaction between the components, e.g., hydrogen bonding.
            \item The sign of $\beta$ reflects the details of molecular packing. For example, if molecules pack very efficiently into real space (as opposed to the idealized equal volume of lattice cells), then there is probably \emph{more} entropy of mixing than lattice theory would predict and $\beta<0$.
        \end{itemize}
        \item The signs of the various components give us insight into the phase diagram.
        \begin{itemize}
            \item Consider the case of $\alpha<0$ and $\beta>0$. Herein, molecules of different components enthalpically attract to each other, and fit less well into space when they're around each other than we might ideally expect. Thus, there should be a temperature at which we rupture the enthalpic bonds and allow the components to pack more efficiently (i.e., with themselves). Thus, we may expect an LCST on the phase diagram.
        \end{itemize}
        \item Last note: $\chi_\text{eff}$ often depends on concentration $\phi_i$.
        \begin{itemize}
            \item This can lead to asymmetry in the $\Delta G_M(\phi_i)$ curves.
            \item It can also complicate experimental measurement of $\chi_\text{eff}$.
        \end{itemize}
    \end{itemize}
\end{itemize}
