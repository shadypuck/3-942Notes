...
\begin{itemize}
    ...
    \item Let's start by analyzing the SALS caused by a dilute gas of small particles in a vacuum.
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item This equation --- $R=KMc$ --- is foundational.
        \item Note that, per our expression for $\alpha$ in a polymer-solvent solution, we have in this case that
        \begin{equation*}
            K = \frac{2\pi^2n_0^2(\dv*{n}{c})^2}{\lambda^4N_\text{A}}
        \end{equation*}
    \end{itemize}
    \item From here through the rest of the derivation, what are we doing and how did we introduce $\prb{\delta c}^2$??
    \item Scattering from fluctuations: Debye's idea.
    \begin{figure}[h!]
        \centering
    ...
    \end{figure}
    \begin{itemize}
        \item Let's bin various areas analogous to lattice theory. Some bins will have pure solvent, and others will have polymer per solvent.
        \item On average, each cell has the same number of solvent molecules.
        \begin{itemize}
            \item But fluctuations in solvent density lead to weak scattering.
        \end{itemize}
        \item Fluctuations in the number of solute molecules (chains) will give rise to significant scattering.
        \item The osmotic forces that try to bring solvent molecules into polymer "cells"...
        \item Lastly, polymer chains are large and cannot be treated as point scatterers. Thus, $P(\theta)\neq 1$.
    \end{itemize}
    \item The features of excess scattering.
    \begin{enumerate}
        ...
        \item Lastly, we should take into account how the Flory-Huggins osmotic pressure depends on different parameters, namely $\Mw$ and $\chi$. In particular, we get the left equation below which allows us to calculate the right derivative below.
        \begin{align*}
    ...
    \begin{equation*}
        \Delta R = \frac{2\pi^2n_0^2(\dv*{n}{c})^2c}{\lambda^4N_\text{A}\left( \frac{1}{M}+2A_2c+\cdots \right)}
